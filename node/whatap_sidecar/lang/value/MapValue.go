...
func (this *MapValue) GetInt(key string) int32 {
	o := this.table.Get(key)
	if o != nil {
		if o.(Value).GetValueType() == VALUE_DECIMAL {
			t := o.(*DecimalValue)
			return int32(t.Val)
		} else if o.(Value).GetValueType() == VALUE_DECIMAL_INT {
			t := o.(*IntValue)
			return t.Val
		}
	}
...
func (this *MapValue) GetLong(key string) int64 {
	...
	if o != nil {
		if o.(Value).GetValueType() == VALUE_DECIMAL {
			t := o.(*DecimalValue)
			return t.Val
		} else if o.(Value).GetValueType() == VALUE_DECIMAL_LONG {
			t := o.(*LongValue)
			return t.Val
		}
	}
...
func (this *MapValue) String() string {
	...
	for keys.HasMoreElements() {
		...
		buf.WriteString(fmt.Sprint("(", value.GetValueType(), ")"))
	}
	buf.WriteString("}")
	return buf.String()
}
