...
import (
	...
	"gitlab.whatap.io/hsnam/focus-agent/whatap/util/uuidutil"
)
...
func (this *EventPack) LevelString() string {
	switch this.Level {
	case FATAL:
		return "fATAL"
	case WARNING:
...
func (this *EventPack) ToString() string {
	return fmt.Sprintf("ALERT %s %s %s %s %s ", this.AbstractPack.ToString(), this.LevelString(), this.Title, this.Message, this.Attr.ToString())
