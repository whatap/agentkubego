...
import (
	...
	"github.com/whatap/kube/node/src/whatap/util/iputil"
	"github.com/whatap/kube/node/src/whatap/util/logutil"
	"github.com/whatap/kube/node/src/whatap/util/procutil"
...
func measureProcessPerformance() *map[string]*ProcessInfo {
	...
	for i := 0; i < len(filesInfo) && err == nil; i++ {
		...
		if whatap_config.GetConfig().CollectProcessPssEnabled && stringutil.StringInSlice(pinfo.Cmd1, whatap_config.GetConfig().CollectProcessPssTargetList) {
			procutil.ParseKeyValue(pid, "smaps", func(key string, val int64) {
				if key == "Pss" {
