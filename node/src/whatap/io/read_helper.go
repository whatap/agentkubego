...
func (in *NetReadHelper) ReadLong() (int64, error) {
	if b, err := in.ReadBytes(8); err == nil {
		return ToLong(b, 0), nil
	} else {
		return 0, err
	}
}

func (in *NetReadHelper) ReadIntBytesLimit(max int32) ([]byte, error) {
	sz, err := in.ReadInt()
	if err != nil {
		return nil, err
	}
...
func (in *NetReadHelper) ReadShort() (int16, error) {
	...
	if b != nil {
		return ToShort(b, 0), nil
	}
	return 0, err
}

func (in *NetReadHelper) ReadUnsignedShort() (uint16, error) {
	b, err := in.ReadBytes(2)
	if b != nil {
		return ToUshort(b, 0), nil
	}
	return 0, err
}
