...
func (this *IntKeyLinkedMap) create(key int32) interface{} {
	panic("not implemented create()")
	//throw new RuntimeException("not implemented create()");
}

func (this *IntKeyLinkedMap) Intern(key int32) interface{} {
	return this._intern(key, PUT_LAST)
}

func (this *IntKeyLinkedMap) _intern(key int32, m PUT_MODE) interface{} {
	this.lock.Lock()
	defer this.lock.Unlock()
	tab := this.table
	index := this.hash(key) % uint(len(tab))
	...
	e := NewIntKeyLinkedEntry(key, value, tab[index])
	tab[index] = e
	switch m {
	case PUT_FORCE_FIRST:
		fallthrough
