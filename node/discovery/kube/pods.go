...
import (
	...
	"github.com/whatap/kube/node/src/whatap/util/containerd"
	"github.com/whatap/kube/node/src/whatap/util/crio"
	"github.com/whatap/kube/node/src/whatap/util/docker"
	"github.com/whatap/kube/node/src/whatap/util/kubeapi"
...
var (
	...
	isContainerD         = containerd.CheckContainerdEnabled()
	isDocker             = docker.CheckDockerEnabled()
	isCrio               = crio.CheckCrioEnabled()
	containerIdExists, _ = whatap_cgroup.CheckIfContainerIdExists()
	podsMap              map[string]SimplePodInfo
...
func executeInjection(podInfo *SimplePodInfo) {
	...
	for i, containerInfo := range podInfo.ContainerInfos {
		...
		whataJavaAgentPathFromRuntimeAPI, err := InspectWhatapAgentPath(containerInfo.ContainerId)
		if err != nil {
		...
		if whataJavaAgentPathFromRuntimeAPI != "" {
			if !isValidAgentPath(whataJavaAgentPathFromRuntimeAPI) {
...
func checkAndExecuteInjection(i int, podInfo *SimplePodInfo, containerInfo SimpleContainerInfo, agentPath, source string) bool {
	if agentPath != "" {
		agentPathIsValid := isValidAgentPath(agentPath)
...
func InspectWhatapAgentPath(containerID string) (string, error) {
	...
	if isDocker {
		logutil.Debugf("execDebug", "isDocker=%v\n", isDocker)
