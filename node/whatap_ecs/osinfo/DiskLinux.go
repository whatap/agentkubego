...
func DiskSectorSizeBytes(logicalDisk string) int64 {
	...
	filesInfo, err := ioutil.ReadDir(filepath.Join(conf.NodeVolPrefix, PathSysBlock))
	...
	path := filepath.Join(conf.NodeVolPrefix, PathSysBlock, disk, "queue", "hw_sector_size")
...
func parseNativeDiskIO(diskQuotas []DiskPerf) ([]DiskPerf, error) {
	...
	conf := config.GetConfig()
	filepath := path.Join(conf.NodeVolPrefix, "/proc/1/diskstats")
...
func getCpuGuestJiff() (int64, error) {
	conf := config.GetConfig()
	filepath := path.Join(conf.NodeVolPrefix, "/proc/uptime")
	file, err := os.Open(filepath)
	if err != nil {
		return 0, err
	}
...
func getDisk() ([]DiskPerf, error) {
	conf := config.GetConfig()
	filepath := path.Join(conf.NodeVolPrefix, "/proc/1/mountinfo")
	file, err := os.Open(filepath)
	if err != nil {
		return nil, err
	}
