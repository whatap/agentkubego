...
FROM --platform=$BUILDPLATFORM public.ecr.aws/docker/library/golang:1.25.11 AS whatap_cadvisor_helper_build
...
FROM --platform=$BUILDPLATFORM public.ecr.aws/docker/library/golang:1.25.11 AS whatap_debugger_build
ARG TARGETOS
ARG TARGETARCH
...
FROM --platform=${BUILDPLATFORM} public.ecr.aws/docker/library/golang:1.25.11-alpine3.22 AS whatap_control_plane_helper_build
