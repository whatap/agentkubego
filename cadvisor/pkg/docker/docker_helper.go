...
func GetContainerStats(containerId string) (string, error) {
	var content string
	// Get container parameters and fetch stats
	err := GetContainerParams(HOSTPATH_PREFIX, containerId, func(name string, cgroupParent string,
		restartCount int, pid int, memoryLimit int64) error {

		// Fetch the restart count from cgroup
		restartCount, _, _ = whatap_cgroup.GetContainerRestartCount(containerId)

		// Fetch detailed container stats
		var statserr error
		content, statserr = getContainerStatsEx(HOSTPATH_PREFIX, containerId, name, cgroupParent,
			restartCount, pid, memoryLimit)
		return statserr
	})

	// If there is an error, fallback to basic container stats
	if err != nil {
