...
func GetContainerInspect(containerId string) (string, error) {
	cinfo := whatap_model.ContainerInfo{}
	cinfo.ID = containerId
	restartCount, _, errrst := whatap_cgroup.GetContainerRestartCount(containerId)
	if errrst != nil {
		return "", errrst
	}
	...
	for _, v := range cexts {
		if "github.com/containerd/cri/pkg/store/container/Metadata" == v.GetTypeUrl() {
			...
			csm := whatap_model.ContainerdSandboxMetadata{}
	...
	contJson, err := json.Marshal(cinfo)
	if err == nil {
...
func GetContainerStats(containerId string) (string, error) {
	...
	pid, _ := proc.GetContainerPid(containerId)
	memoryLimit := int64(0)
...
func InspectContainerdImageForWhatapPath(containerID string) (string, error) {
	container, ctx, err := whatap_client.LoadContainerD(containerID)
	if err != nil {
