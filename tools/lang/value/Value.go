...
func CreateValue(code byte) Value {
	return CreateValueRaw(code).(Value)
}

func CreateValueRaw(code byte) interface{} {
	switch code {
	case VALUE_NULL:
		return NewNullValue()
	case VALUE_BOOLEAN:
...
func WriteListValue(out *io.DataOutputX, val *ListValue) *io.DataOutputX {
	out.WriteByte(val.GetValueType())
	val.Write(out)
	return out
}
...
func ReadListValue(in *io.DataInputX) *ListValue {
	t := in.ReadByte()
	v := CreateValueRaw(byte(t)).(*ListValue)
	v.Read(in)
	return v
}

func ReadMapValue(in *io.DataInputX) *MapValue {
	t := in.ReadByte()
	v := CreateValueRaw(byte(t)).(*MapValue)
	v.Read(in)
	return v
}
