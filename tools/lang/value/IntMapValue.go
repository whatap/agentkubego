...
import (
	...
	"github.com/whatap/kube/tools/util/hmap"
)
...
func (this *IntMapValue) GetRaw(key int32) interface{} {
...
func (this *IntMapValue) PutList(key int32, value ListValue) {
	this.table.Put(key, value)
}
...
func (this *IntMapValue) Write(dout *io.DataOutputX) {
	...
	for keys.HasMoreElements() {
		key := keys.NextInt()
		dout.WriteInt(key)
		if value, ok := this.table.Get(key).(Value); ok {
			WriteValue(dout, value)
		} else if value, ok := this.table.Get(key).(*ListValue); ok {
			WriteListValue(dout, value)
		}
	}
}
...
func (this *IntMapValue) ToString() string {
